...
router.post(
  adminPath() + '/settings',
  auth,
  upload.fields([{name:'logoFile',maxCount:1},{name:'faviconFile',maxCount:1},{name:'ogImageFile',maxCount:1}]),
  (req,res)=>{

    const payload = { ...req.body };
    if(req.files && req.files.logoFile) payload.logoUrl = fileUrl(req.files.logoFile[0]);
    if(req.files && req.files.faviconFile) payload.faviconUrl = fileUrl(req.files.faviconFile[0]);
    if(req.files && req.files.ogImageFile) payload.ogImageUrl = fileUrl(req.files.ogImageFile[0]);
...
router.post(
  ...
  (req,res)=>{
    ...
    articles.unshift({
      ...
      coverUrl:
        fileUrl(req.file) || req.body.coverUrl ||
...
router.post(
  ...
  (req,res,next)=>{
    ...
    articles[index] = {
      ...
      coverUrl:
        fileUrl(req.file) || req.body.coverUrl || articles[index].coverUrl ||
        '/images/default-cover.svg',
